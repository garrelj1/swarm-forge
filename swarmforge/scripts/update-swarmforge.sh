...
# The PM prompt is pack-independent — it decides at startup whether it is an
# in-swarm role from whether swarmforge.conf has a `window PM` line — so it
# lives on main and is kept current from here regardless of the selected pack.
